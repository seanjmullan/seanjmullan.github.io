...
-   __New java.security.AsymmetricKey interface__
    ...
    A new standard interface named `java.security.AsymmetricKey` which is a
    subinterface of `java.security.Key` and represents an asymmetric key,
    which can either be a private or public key. The existing
    `java.security.PublicKey` and `java.security.PrivateKey` classes have been
    retrofitted to be subinterfaces of `AsymmetricKey`.

    The `Asymmetric` interface provides a default implementation of the
    `getParams` method which returns `null`.

    All asymmetric key subclasses now have a `getParams` method. This has two
    primary benefits:

     - Application code can retrieve parameters without using `instanceof` to
       first determine the subclass.
     - Many asymmetric algorithm parameters can be represented using the
       existing `NamedParameterSpec` subclass. As future asymmetric algorithms
       are introduced, the `Asymmetric` interface will allow earlier versions
       of Java SE to more easily support new asymmetric algorithms without
       requiring new algorithm-specific APIs.
...
-   __The jdk.crypto.ec module has been subsumed into java.base__
    ...
    All of the code in the `jdk.crypto.ec` module has been moved into the
    `java.base` module. This includes the SunEC provider. The `jdk.crypto.ec`
    module still exists, but it is now empty.

    This change will make it easier for users to deploy applications that
    depend on elliptic curve algorithms.
